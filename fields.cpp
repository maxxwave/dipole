      ...
      for (int i=0; i<create::index; i++){
        ...
        for (int j=0; j<create::index; j++){
           ...
           if(i!=j){
              ...
              st::H_dip[i].x += pf * (3.0* rij_x * (rij_x * st::atom[j].sx) - st::atom[j].sx) / (rij*rij*rij);
              st::H_dip[i].y += pf * (3.0* rij_y * (rij_y * st::atom[j].sy) - st::atom[j].sy) / (rij*rij*rij);
              st::H_dip[i].z += pf * (3.0* rij_z * (rij_z * st::atom[j].sz) - st::atom[j].sz) / (rij*rij*rij);
           } // end of if 
